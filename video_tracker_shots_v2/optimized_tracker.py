...
class ShotsVideoProcessor(OptimizedVideoProcessor):
    ...
    def process_video_with_shots(self, input_path: str, output_path: str):
        """Обрабатывает видео с разбивкой на шоты и сохранением фото"""
        logger.info(f"🎬 Начинаем обработку с разбивкой на шоты: {input_path}")
        
        # Читаем анализ сцен
        scenes_df = pd.read_csv("scene_analysis_optimized.csv")
        ...
        for idx, scene in scenes_df.iterrows():
            ...
            logger.info(f"🎬 Обрабатываем шот {idx}: кадры {start_frame}-{end_frame}")
            
            # Обрабатываем шот: сохраняем видео, трекинг и фото в одном проходе
            shot_csv_data = self._process_single_shot(input_path, start_frame, end_frame, idx)
            csv_data.extend(shot_csv_data)
        
        # Сохраняем общий CSV файл
        if csv_data:
            df = pd.DataFrame(csv_data)
            df.to_csv("people_tracking_results.csv", index=False)
            logger.info(f"📊 Сохранен CSV файл с {len(csv_data)} записями")
    ...
    def _process_single_shot(self, input_path: str, start_frame: int, end_frame: int, shot_id: int):
        ...
        # Создаем видеописатель для шота
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        ...
        # Создаем папку для фото шота
        shot_photos_dir = self.photos_dir / f"shot_{shot_id:03d}"
        shot_photos_dir.mkdir(parents=True, exist_ok=True)
        ...
        for frame_num in range(start_frame, end_frame + 1):
            ...
            result_frame = self._draw_results(frame, tracked_boxes, frame_num, fps)
            
            # Записываем кадр в видео шота
        ...
        logger.info(f"✅ Шот {shot_id} обработан: видео сохранено, фото сохранены")
        return csv_data
...
def main():
    ...
    processor.process_video_with_shots("mister-i-missis-smit-2005_1.mkv", "output_video_optimized.mp4")
